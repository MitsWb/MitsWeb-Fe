    ...
    if (!formLoading) {
      ...
      files.forEach((file) => {
        if (types.every((type) => file.type !== type)) {
          setErr(`"${file.type}" is not a supported format(jpg and png)`);
          return false;
    ...
    <div className="flex flex-col w-full bg-grey-lighter">
      <div className="flex flex-wrap items-center">
        ...
        >
          ...
          <span className="ml-1 text-sm font-bold">Add Image</span>
          <input
            type="file"
            className="hidden"
        ...
        <div className="flex px-4 flex-wrap">
          {prevImages.map((image, i) => previewImage(image, i, false))}
