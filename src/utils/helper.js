export function getAppliedFilters(options = null, all = false) {
  ...
  if (appliedFilters) {
    if (options) {
      currentForm.category = options.category.includes(appliedFilters.category)
        ? appliedFilters.category
        : options.category[0];
      currentForm.district = options.district.includes(appliedFilters.district)
        ? appliedFilters.district
        : options.district[0];
      currentForm.minimum =
        appliedFilters.minimum && appliedFilters.minimum >= options.minimum
          ? appliedFilters.minimum
          : options.minimum;
      currentForm.maximum =
        appliedFilters.maximum && appliedFilters.maximum <= options.maximum
